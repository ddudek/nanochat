#!/usr/bin/env bash
# Minimal-ish nanochat speedrun tuned for 2x RTX 3090
# Put this file in the root of the karpathy/nanochat repo and run:
#   bash speedrun_2x3090.sh
#
...
# How many GPUs to use (defaults to 2 for 2x3090)
...
# DEVICE_BATCH_SIZE * CONTEXT_SIZE * 2 * X
# X = 7
...
export WANDB_RUN="nano-chat-${MODEL_TAG}"
...
if [[ DO_PRETREINING -eq 1 ]]; then
  ...
  time torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" -m scripts.base_eval
fi
...
python -m nanochat.report || echo "nanochat.report exited non-zero; check logs."
